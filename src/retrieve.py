...
def len_of_chat_template(tokenizer, cfg: Configs):
    message = [
...
def retrieve(retriever: Optional[Retrieval], llm, tokenizer, messages, max_seq_length, cfg: Configs, topk: int=5):
    prompt_tokens = len_of_tokens(tokenizer, messages)
    chat_template_tokens = len_of_chat_template(tokenizer, cfg) + 10
    ...
    if max_response_tokens < 0: 
        ...
        return None
    if rag_response_threshold > cfg.rag_response_threshold:
